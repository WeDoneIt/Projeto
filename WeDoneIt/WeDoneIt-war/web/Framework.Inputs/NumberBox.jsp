...
<div class="input-group input-group-sm ajuste">
    <input type="text" class="form-control number" id="<c:out value='${param.name}'/>" name="<c:out value='${param.name}'/>" style="margin-left: 1%; width: 90%; margin-right: 1%;"/><div id="NOTY_<c:out value='${param.name}'/>" style="margin-left:10px; margin-right: 10px;"></div>
</div>
   ...
   $("#NOTY_<c:out value='${param.name}'/>").fadeOut();
    $(document).ready(function () {
        ...
        $("#<c:out value='${param.name}'/>").keypress(function (e) {
            ...
            var virgula = $("#<c:out value='${param.name}'/>").val();
